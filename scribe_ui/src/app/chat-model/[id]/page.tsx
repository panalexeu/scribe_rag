...
export default function Page() {
    ...
    async function handleSubmit() {
        ...
        try {
            const putRequest = ChatModelPutModel.parse({
                name: name,
                api_key_credential_id: apiKeyCredential.id,
                temperature: !temperature ? null : Number(temperature),
                top_p: !topP ? null : Number(topP),
                base_url: !baseURL || (baseURL && baseURL.isEmpty) ? null : baseURL,
                max_tokens: !maxTokens ? null : Number(maxTokens),
                max_retries: !maxRetries ? null : Number(maxRetries),
                stop_sequences: stopSequences.length === 0? null : JSON.stringify(stopSequences)
            })
    ...
    async function fetchChatModel() {
        try {
            ...
            if (response.status === 200) {
                const data: ChatModelResponseModel = await response.json();
                setName(data.name);
                ...
                setStopSequences(!data.stop_sequences ? [] : JSON.parse(data.stop_sequences))
            } else {
                setSnackbarMessage(`something went wrong 😢, status code: ${response.status}`);
                setOpenSnackbar(true);
            }
