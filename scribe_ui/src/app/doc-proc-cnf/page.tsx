...
import {DocProcCnfResponseModel} from './models';
...
export default function Page() {
    ...
    const [items, setItems] = useState<DocProcCnfResponseModel[]>([]);
    ...
    async function handleDelete(id: number) {
        try {
            ...
            if (response.status == 204) {
                setSnackbarMessage(`doc processing cnf with the id: ${id} was successfully deleted 🥳`);
                setOpenSnackbar(true);
                await fetchCount();
                await fetchItems();
            } else {
    ...
    return (
        ...
        >
            ...
            <Box display={'flex'} width={'100%'}>
                ...
                <Tooltip title={'add new doc-proc-cnf'}>
                    <IconButton
                        onClick={() => router.push('/doc-proc-cnf/add')}
                        size={'small'}
                        sx={{ml: 'auto'}}
                    >
                        <AddIcon/>
            ...
            >
                {
                    items.map((item, _) => (
                        ...
                        >
                            ...
                            {/* ACTIONS */}
                            {/* DELETE */}
                            <Tooltip title={'delete doc-proc-cnf'}>
                                <IconButton
                                    sx={{ml: 'auto'}}
                                    size={'small'}
                            ...
                            {/* OPEN */}
                            <Tooltip
                                title={'open doc-proc-cnf'}
                                onClick={() => router.push(`/doc-proc-cnf/${item.id}`)}
                            >
